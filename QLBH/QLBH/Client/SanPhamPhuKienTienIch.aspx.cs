...
{
	public partial class SanPhamPhuKienTienIch : System.Web.UI.Page
	{
        ...
        void getData()
        {
            ...
            List<HangHoa> data = db.HangHoa.OrderBy(x => x.MaHangHoa).Skip(20).Take(3).ToList<HangHoa>();
            rpVanPhongPham_01.DataSource = data;
            rpVanPhongPham_01.DataBind();

            List<HangHoa> data_2 = db.HangHoa.OrderBy(x => x.MaHangHoa).Skip(23).Take(3).ToList<HangHoa>();
            rpVanPhongPham_02.DataSource = data_2;
            rpVanPhongPham_02.DataBind();

            List<HangHoa> data_3 = db.HangHoa.OrderBy(x => x.MaHangHoa).Skip(26).Take(3).ToList<HangHoa>();
            rpVanPhongPham_03.DataSource = data_3;
            rpVanPhongPham_03.DataBind();

            List<HangHoa> data_4 = db.HangHoa.OrderBy(x => x.MaHangHoa).Skip(29).Take(3).ToList<HangHoa>();
